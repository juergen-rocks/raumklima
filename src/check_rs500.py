...
def handle_request(args: argparse.Namespace):
    ...
    try:
        ...
        if (
            any(
                [
                    args.min_temp is not None,
                    args.max_temp is not None,
                    args.min_warn_temp is not None,
                    args.max_warn_temp is not None,
                ]
            )
            and raw_value_temp is None
        ):
            print(
                "{}: Should check temperature, but temperature is unknown; channel = {} -> temp = {}, humi = {}".format(
                    EXIT_WORD_CRIT, args.channel, temp, humi
                )
            )
            exit(EXIT_CODE_CRIT)
        if (
            any(
                [
                    args.min_hum is not None,
                    args.max_hum is not None,
                    args.min_warn_hum is not None,
                    args.max_warn_hum is not None,
                ]
            )
            and raw_value_humi is None
        ):
            print(
                "{}: Should check humidity, but humidity is unknown; channel = {} -> temp = {}, humi = {}".format(
                    EXIT_WORD_CRIT, args.channel, temp, humi
                )
            )
            exit(EXIT_CODE_CRIT)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="check_rs500", description="Hole RS500 Messwerte", epilog="Nutzbar als Icinga-Plugin"
    )
    parser.add_argument("-c", "--channel", type=int, required=True, help="Kanal-Nummer (Pflicht)")
    parser.add_argument(
        "--min-temp", type=float, help="Mindesttemperatur (numerische Werte) - Unterschreitung ist Critical"
    )
    parser.add_argument(
        "--max-temp", type=float, help="Maximalmaximaltemperatur (numerische Werte) - Überschreitung ist Critical"
    )
    parser.add_argument(
        "--min-warn-temp", type=float, help="Mindesttemperatur Warnung (numerische Werte) - Unterschreitung ist Warnung"
    )
    parser.add_argument(
        "--max-warn-temp", type=float, help="Maximaltemperatur Warnung (numerische Werte) - Überschreitung ist Warnung"
    )
    parser.add_argument(
        "--min-hum", type=int, help="Mindestluftfeuchte (numerische Werte) - Unterschreitung ist Critical"
    )
    parser.add_argument(
        "--max-hum", type=int, help="Maximalluftfeuchte (numerische Werte) - Überschreitung ist Critical"
    )
    parser.add_argument(
        "--min-warn-hum", type=int, help="Mindestluftfeuchte Warnung (numerische Werte) - Unterschreitung ist Warnung"
    )
    parser.add_argument(
        "--max-warn-hum", type=int, help="Maximalluftfeuchte Warnung (numerische Werte) - Überschreitung ist Warnung"
    )
